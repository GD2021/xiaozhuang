...
------------------
> [含补丁软件包](https://github.com/main-studio/TyporaCrack)
...
# [推荐]HEUKMSActivator （office激活首选且可安装续期） 
...
# AudioRelay（通过局域网让手机作为电脑的麦克风）
https://audiorelay.net/
---
1、手机电脑都下载客户端
2、确保在同一局域网下，手机作为服务器，而电脑通过局域网ip连接作为播放器（模式“麦克风”）
...
# WindowTabs（一款将开源的标签式管理窗口，快速切换窗口，相关窗口聚合在一起）
...
# [推荐]360压缩-国际版（页面简洁美观无广告，拖拽解压不放临时快人一步！）
https://www.360totalsecurity.com/zh-cn/360zip
----
> [蓝奏云下载](https://zjazn.lanzoub.com/i61w13bslhxe "官网下载不了时，下载慢时使用")
...
## 有两个分支 master与新无master历史的新分支yyy-branch，如何将yyy-branch分支的文件以master基础上修改来进行合并到master呢？
git checkout master
git pull origin master
git merge yyy-branch --allow-unrelated-histories
## 建议配置的git
```bash
...
- 命令创建文件或文件夹的软链接：`mklink /D "D:\data\users\zhuangjie\Desktop\word_files"  "D:\data\word_files"`
...
windows杀端口对应的进程（以`8080`端口示例）：`for /f "tokens=5" %i in ('netstat -ano ^| findstr :8080') do taskkill /f /pid %i`
...
# [推荐]驱动总裁（电脑安装驱动，驱动不限速下载）
https://www.sysceo.com/software-softwarei-id-258.html
----
驱动精灵（检测品牌驱动下载驱动，不更新了）: https://aichunjing.lanzoui.com/b620774
...
----
[立即下载](https://www.sordum.org/downloads/?st-windows-update-blocker)
